...
I made this project as an entry to the [WIZnet ioNIC Contest](https://maker.wiznet.io/contest/ionic) using the [W55RP20-EVB-PICO](https://docs.wiznet.io/Product/ioNIC/W55RP20/w55rp20-evb-pico). Most IoT devices use wireless communication nowadays, so it's refreshing to see a microcontroller with a built-in wired Ethernet controller. 
...
For intrusion detection, a reed switch can be used. For motion detection, I used a PIR sensor. The brain of the system is the W55RP20-EVB-PICO. The components need to be connected in the following way:
...
I placed the downloaded WIZnet libraries in a folder named `c-sdk` and I created a folder with the `main.c`, `MakeLists.txt`, `webpage.h` files and the `build` folder in a directory on the same level as `c-sdk`:
```
.
├── c-sdk
...
Since the microcontroller does not have a built-in RTC to keep track of time, the milliseconds elapsed will be used to measure the last time a movement was detected and the door state (`OPEN` or `CLOSED`) change's last time. Then, we can use JavaScript and the browser's time to interpret and display the results. Let's look at the webpage first. Here, we can use a single file with HTML+CSS+JS included. The main logic is that there is a map in the website code which is updated on the server-side when serving the webpage:
``` js
        const serverData = {
            lastMotionMs: 2000,
            doorState: "OPEN",
...
```
As seen on the schematic earlier, GPIO 2 is connected to the PIR sensor, and GPIO 3 is connected to the reed switch. We also need two long variables to store the millisecond time of the last movement and door change. Finally, we need a boolean variable to store the current state of the door.

The main function starts with defining a couple of variables which will be used as temporary placeholders for the formatted strings. These are needed as `snprintf` puts a terminating null character at the end, making the website return a partial result.
``` c
int main()
{
    /* Initialize */
...
```
Next, we can configure the GPIO pins. The PIR pin is driven by the PIR sensor, but for the reed switch, we need to enable pullup on the input so that when the door is closed (the magnet is next to the switch) and the circuit is closed, it is pulled down which we can detect.
``` c
...
```
Detection is done by configuring interrupt logic for when the logic level on the pins change:
``` c
...
```
Then, we can define an infinite loop and inside, we can copy the formatted strings to the temporary variables. Then, those can be copied to the `WEBPAGE_HTML` string so that when the webpage is loaded from a browser, the up-to-date values are present in the webpage source code:
``` c
    while (1){
...
The final step is to mount the finished electronics next to a door and use it to check remotely the status of your home:
